...
# plot overall
fig = plt.figure()
plt.plot(ea_train_val_value[['top_n_var']], ea_train_val_value[['weighted_auc_train']], color='olive', linewidth=2, label="weighted auc on training set")
plt.plot(ea_train_val_value[['top_n_var']], ea_train_val_value[['weighted_auc_val']], color='red', linewidth=2, label="weighted auc on validation set")
#plt.plot(236,0.53, marker='o', markersize=3, color='red', label="weighted auc on test set")
plt.ylim(0,1)
plt.legend()

# add axs
plt.title("EA No Drug Model Training and Validation AUC score")
plt.xlabel("Features (sorted)")
plt.ylabel("Weighted AUC Score")
plt.savefig("plots/ea_no_drug_training_and_val_weighted_auc.png")
plt.close(fig)
