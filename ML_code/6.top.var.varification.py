...
aa_rank=pd.read_csv("aa.ci_plus_average.final.txt", sep='\t')

#get the top 50 variables
aa_rank['var'][:51]  # give top 50 variables

# limit the dataset to only top variables and re-train model with same parameters --- the reason is assuming the unselected variables don't matter much, with the same parameters and same settings, the model could be trained to perform better
top_var=int(sys.argv[1])

aa_train_x=aa_train_x[aa_rank['var'][:top_var]]
aa_val_x=aa_val_x[aa_rank['var'][:top_var]]
aa_test_x=aa_test_x[aa_rank['var'][:top_var]]
...
def get_model(X_train):
       model = Sequential()
       model.add(Dense(hidden_layer1, input_dim=X_train.shape[1], activation='relu',kernel_regularizer=regularizers.l2(0.0015)))
       #maybe use BatchNormalization(axis = 3)(x)
       model.add(Dense(hidden_layer2, activation='relu',kernel_regularizer=regularizers.l2(0.0015)))
       model.add(Dense(1,activation="sigmoid",kernel_regularizer=regularizers.l2(0.0015)))
              #compile model
...
for j, (train_idx, val_idx) in enumerate(folds):
       ...
       #print(filepath)
       callbacks = [keras.callbacks.EarlyStopping(monitor='val_weighted_f1', min_delta=0.0005, patience=10, verbose=0, mode='max', baseline=0.9)]
...
# get only the weighted_auc and weighted_f1 from training and testing
result=[top_var]+model.evaluate(aa_train_x.values, aa_train_y.values,batch_size=len(aa_train_y.values))[1:3] +model.evaluate(aa_val_x.values, aa_val_y.values,batch_size=len(aa_val_y.values))[1:3]  +model.evaluate(aa_test_x.values, aa_test_y.values,batch_size=len(aa_test_y.values))[1:3]


# pre save a file with this name with only the header
with open('ML/dnn/m1/logs/4.aa.var_selection_validation.txt', 'a') as savefile:
	savefile.write('\t'.join(map(str, result)) +'\n')
